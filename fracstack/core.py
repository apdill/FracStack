...
def measure_dimension(input_array,
                      ...
                      n_bootstrap=1000,
                      bootstrap_seed=None,
                      include_lacunarity=False,
                      method='gliding',
                      lacunarity_kwargs=None):
    """
    ...
    bootstrap_seed : int, optional
        ...
        random state.
    include_lacunarity : bool, default False
        Whether to compute lacunarity alongside the fractal dimension. The
        specific method (gliding or non-gliding) is controlled via
        `lacunarity_kwargs`.
    lacunarity_kwargs : dict, optional
        Additional keyword arguments forwarded to `lacunarity_boxcount`
        (e.g., custom `sizes`, `method='box'`, padding behaviour, or offset
        controls).
    ...
    dict
        ...
        - 'lacunarity_sizes' : np.ndarray or None - Evaluated box sizes
        - 'lacunarity_values' : np.ndarray or None - Lacunarity Λ(L) at each size
        - 'lacunarity_mean_mass' : np.ndarray or None - Average occupied mass per size
        - 'lacunarity_variance' : np.ndarray or None - Mass variance per size
        - 'lacunarity_windows' : np.ndarray or None - Number of contributing windows
    ...
    if mode == 'D2':
        corr_kwargs = {} if lacunarity_kwargs is None else dict(lacunarity_kwargs)
        correlation_method = corr_kwargs.get('method', method)
        if correlation_method == 'gliding':
            corr_kwargs.setdefault('pad_factor', None)
            corr_kwargs.setdefault('min_size', min_size)
            corr_kwargs.setdefault('max_size', max_size)
            corr_kwargs.setdefault('num_sizes', num_sizes)
            corr_kwargs.setdefault('method', 'gliding')
            corr_kwargs.setdefault('use_optimization', use_optimization)
            if seed is not None:
                corr_kwargs.setdefault('seed', seed)
            sizes_arr, corr_values, _ = correlation_dimension(
                padded_array,
                **corr_kwargs,
            )
            sizes = sizes_arr
            counts = corr_values.tolist()
        else:
    ...
    embedding_correction = 0
    if mode == 'D2':
        embedding_correction = 2 if correlation_method == 'gliding' else 0
    ...
    valid_sizes, valid_counts, d_value, fit, r2, ci_low, ci_high = compute_dimension(
        ...
        embedding_dim=embedding_correction,
    )
    ...
    if include_lacunarity:
        lac_kwargs = {} if lacunarity_kwargs is None else dict(lacunarity_kwargs)
        if 'pad_factor' not in lac_kwargs:
            lac_kwargs['pad_factor'] = None
        if 'sizes' not in lac_kwargs:
            lac_kwargs.setdefault('min_size', min_size)
            lac_kwargs.setdefault('max_size', max_size)
            lac_kwargs.setdefault('num_sizes', num_sizes)

        method = lac_kwargs.get('method', method)
        print(f"Using lacunarity method: {method}")
        lac_kwargs['method'] = method
        if method == 'box':
            lac_kwargs.setdefault('num_offsets', num_offsets)
            if seed is not None:
                lac_kwargs.setdefault('seed', seed)
            lac_kwargs.setdefault('use_min', False)

        lac_sizes, lac_values, lac_mean_mass, lac_variance, lac_windows = lacunarity_boxcount(
            padded_array,
            **lac_kwargs,
        )
    ...
    return {
        ...
        'ci_low': ci_low,
        'ci_high': ci_high,
        'lacunarity_sizes': lac_sizes,
        'lacunarity_values': lac_values,
        'lacunarity_mean_mass': lac_mean_mass,
        'lacunarity_variance': lac_variance,
        'lacunarity_windows': lac_windows,
    }
...
def portfolio_plot(input_array = None,
    ...
    include_lacunarity : bool, default False
        Whether to compute and display lacunarity curves (gliding or box-count,
        depending on `lacunarity_kwargs`).
    show_lacunarity : bool, default False
        Whether to display the lacunarity plot interactively (in addition to saving
        when a save directory is provided).
    lacunarity_kwargs : dict, optional
        Extra keyword arguments forwarded to `measure_dimension` for lacunarity
        computation (passed through to `lacunarity_boxcount`, e.g. `method='box'`,
        custom sizes, padding, or offset controls).
    ...
    3. **Target Consistency Check**: |D0 - target_D0| ≤ D0_D1_threshold (When using min count harder for D0 to be bounded by D1)
    ...
    lac_windows = None
    if include_lacunarity:
        lac_kwargs_local = {} if lacunarity_kwargs is None else dict(lacunarity_kwargs)
        if 'pad_factor' not in lac_kwargs_local:
            lac_kwargs_local['pad_factor'] = None
        if custom_sizes is not None and 'sizes' not in lac_kwargs_local:
            lac_kwargs_local['sizes'] = custom_sizes.tolist()
        else:
            lac_kwargs_local.setdefault('min_size', min_size)
            lac_kwargs_local.setdefault('max_size', max_size)
            lac_kwargs_local.setdefault('num_sizes', num_sizes)
        method_local = lac_kwargs_local.get('method', 'gliding')
        if method_local == 'box':
            lac_kwargs_local.setdefault('num_offsets', num_offsets)
            if seed is not None:
                lac_kwargs_local.setdefault('seed', seed)
            lac_kwargs_local.setdefault('use_min', False)

        lac_sizes, lac_values, lac_mean, lac_variance, lac_windows = lacunarity_boxcount(
            padded_input_array,
            **lac_kwargs_local,
        )
    ...
    if (
        include_lacunarity
        and lac_sizes is not None
        and lac_values is not None
        and hasattr(lac_sizes, "size")
        and lac_sizes.size > 0
    ):
        finite_mask = np.isfinite(lac_values)
        if np.any(finite_mask):
            lac_plot_path = None
            if save_dir is not None and f_name is not None:
                lac_plot_path = os.path.join(save_dir, f"{f_name}_lacunarity.png")
            plot_lacunarity_curve(
                sizes=lac_sizes,
                lacunarity=lac_values,
                mean_mass=lac_mean,
                variance=lac_variance,
                window_counts=lac_windows,
                show=show_lacunarity,
                save_path=lac_plot_path,
                title=f_name,
            )
    ...
    if include_lacunarity and lac_sizes is not None and lac_values is not None:
        image_info_dict.update({
            'lacunarity_sizes': lac_sizes,
            'lacunarity_values': lac_values,
            'lacunarity_mean_mass': lac_mean,
            'lacunarity_variance': lac_variance,
            'lacunarity_windows': lac_windows,
        })
...
def analyze_image(input_array = None,
    ...
    **Note**: This function contains legacy code patterns and may have redundant visualizations.
    ...
    if image_path is not None:
        ...
        input_array = process_image_to_array(image_path, threshold=threshold, invert=invert, edge_detection=edge_detection).astype(np.uint8)
        
        # Log what type of image is being analyzed
        if edge_detection:
            print(f"Analyzing edge-detected version of {f_name}")
        else:
            print(f"Analyzing original version of {f_name}")
    ...
    # Initialize object-related variables to safe defaults to avoid UnboundLocalError
    largest_object = None
    smallest_object = None
    labeled_image = None
    largest_diameter = np.nan
    smallest_diameter = np.nan

    if plot_objects:
        largest_object, smallest_object, largest_diameter, smallest_diameter, min_feature_width, min_feature_object, labeled_image = find_largest_smallest_objects(
            input_array,
            save_dir=save_path,
            f_name=f_name,
            show=False,
        )
        plot_object_outlines(labeled_image, largest_object, min_feature_object, f_name=f_name, save_dir=save_path,
                           largest_diameter=largest_diameter, smallest_diameter=smallest_diameter, 
                           min_feature_width=min_feature_width)


    fit_info_dict = measure_dimension(input_array,
                                      mode=mode, 
                                      min_size=min_size, 
                                      max_size=max_size, 
                                      num_sizes=num_sizes, 
    ...
    magnificiation_range = np.round(np.log10(max_size) - np.log10(min_size), decimals=2)
    
    legend_info = {
        'min_box_size': min_size,
        'max_box_size': max_size,
        'largest_diameter': largest_diameter,
        'smallest_feature_width': min_feature_width,
        'image_width': input_array.shape[1],
        'image_height': input_array.shape[0],
        'magnificiation_range': magnificiation_range
    }
    
    # Always save the single-plot version (rich legend) for embedding
    plot_scaling_results(
        f_name,
        invert=invert,
        input_array=input_array,
        sizes=fit_info_dict['valid_sizes'],
        measures=fit_info_dict['valid_counts'],
        d_value=fit_info_dict['D'],
        fit=fit_info_dict['fit'],
        r2=fit_info_dict['R2'],
        save=True,
        save_path=save_path,
        show_image=False,
    ...
    # Always save the two-panel version (image + scaling) when save_output=True
    # This ensures the "result" figure is available for the GUI
    if save_output:
        plot_scaling_results(
            f_name,
            invert=invert,
    ...
    # Optionally also display the two-panel version if requested
    elif show_image:
        plot_scaling_results(
            f_name,
            invert=invert,
...
def analyze_images(base_path, save=False, invert=False, threshold=150, min_size=16, max_size=None, num_sizes=100, num_offsets=10, criteria = None, use_min_count=False, seed=None, use_weighted_fit=True):
    ...
    **Note**: This function contains legacy code patterns and always shows visualizations.
    ...
    for f_name in tqdm(os.listdir(base_path), desc='Doing boxcounts...'):
        ...
        largest_object, smallest_object, largest_diameter, smallest_diameter, min_feature_width, min_feature_object, labeled_image = find_largest_smallest_objects(input_array)
        ...
        show_image_info(fname=f_name, invert=invert, d_value=fit_info_dict['D'], input_array=input_array, sizes=fit_info_dict['valid_sizes'], save=save, save_path=save_path, largest_diameter=largest_diameter, min_feature_width=min_feature_width)
